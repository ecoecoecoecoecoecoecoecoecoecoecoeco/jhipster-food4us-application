  ...
  return (
    <div>
      <h2 id="warehouse-heading" data-cy="WarehouseHeading">
        <Translate contentKey="food4UsApp.warehouse.home.title">Warehouses</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" variant="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="food4UsApp.warehouse.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/warehouse/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="food4UsApp.warehouse.home.createLabel">Create new Warehouse</Translate>
          </Link>
        </div>
      </h2>
        ...
        {warehouseList?.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="food4UsApp.warehouse.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="food4UsApp.warehouse.userProfile">User Profile</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {warehouseList.map(warehouse => (
                <tr key={`entity-${warehouse.id}`} data-cy="entityTable">
                  <td>
                    <Button as={Link as any} to={`/warehouse/${warehouse.id}`} variant="link" size="sm">
                      {warehouse.id}
                    </Button>
                  </td>
                  ...
                  <td>
                    <Translate contentKey={`food4UsApp.WarehouseType.${warehouse.type}`} />
                  </td>
                  <td>
                    {warehouse.userProfile ? <Link to={`/user-profile/${warehouse.userProfile.id}`}>{warehouse.userProfile.id}</Link> : ''}
                  </td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button as={Link as any} to={`/warehouse/${warehouse.id}`} variant="info" size="sm" data-cy="entityDetailsButton">
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="food4UsApp.warehouse.home.notFound">No Warehouses found</Translate>
            </div>
          )
        )}
      </div>
