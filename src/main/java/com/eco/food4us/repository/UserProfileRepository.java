...
public interface UserProfileRepository extends UserProfileRepositoryWithBagRelationships, JpaRepository<UserProfile, Long> {
    ...
    @Query(
        value = "select userProfile from UserProfile userProfile left join fetch userProfile.internalUser",
    ...
    @Query("select userProfile from UserProfile userProfile left join fetch userProfile.internalUser")
