...
public interface ProductRepository extends JpaRepository<Product, Long>, JpaSpecificationExecutor<Product> {
    ...
    @Query(
        value = "select product from Product product left join fetch product.warehouse left join fetch product.supplier",
        countQuery = "select count(product) from Product product"
    )
    Page<Product> findAllWithToOneRelationships(Pageable pageable);

    @Query("select product from Product product left join fetch product.warehouse left join fetch product.supplier")
    List<Product> findAllWithToOneRelationships();

    @Query("select product from Product product left join fetch product.warehouse left join fetch product.supplier where product.id =:id")
