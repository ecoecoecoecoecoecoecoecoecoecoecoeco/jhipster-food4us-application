...
public class UserProfileRepositoryWithBagRelationshipsImpl implements UserProfileRepositoryWithBagRelationships {
    ...
    public Optional<UserProfile> fetchBagRelationships(Optional<UserProfile> userProfile) {
        return userProfile.map(this::fetchAllergyIntolerances);
    }
    ...
    public List<UserProfile> fetchBagRelationships(List<UserProfile> userProfiles) {
        return Optional.of(userProfiles).map(this::fetchAllergyIntolerances).orElse(Collections.emptyList());
    }

    UserProfile fetchAllergyIntolerances(UserProfile result) {
        return entityManager
            .createQuery(
                "select userProfile from UserProfile userProfile left join fetch userProfile.allergyIntolerances where userProfile.id = :id",
                UserProfile.class
            )
    ...
    List<UserProfile> fetchAllergyIntolerances(List<UserProfile> userProfiles) {
        ...
        List<UserProfile> result = entityManager
            .createQuery(
                "select userProfile from UserProfile userProfile left join fetch userProfile.allergyIntolerances where userProfile in :userProfiles",
                UserProfile.class
            )
